...
const AuthModal = ({ isOpen, onClose, mode, onSwitchMode, onLoginSuccess }: AuthModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isOtpStep) {
      try {
        const otpResponse = await fetch('http://localhost:5000/api/auth/verify-email', {
          method: 'POST',
    ...
    const apiUrl = mode === 'signup' ? 'http://localhost:5000/api/auth/signup' : 'http://localhost:5000/api/auth/login';
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        console.log(data)
        onLoginSuccess(data._id || '_id', data.username || 'User', data.token || 'token',data.email||'n210519@rguktn.ac.in');
        onClose();
      }
    } catch (err: any) {
      setError('Error: ' + err.message);
